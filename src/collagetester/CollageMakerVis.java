...
public class CollageMakerVis {
    ...
    class Drawer extends JFrame {
        ...
        public Drawer(Image target, Image collage, int[] ans) {
            ...
            this.target = new BufferedImage(target.width, target.height, BufferedImage.TYPE_INT_RGB);
            this.collage = new BufferedImage(target.width, target.height, BufferedImage.TYPE_INT_RGB);
            this.bndCollage = new BufferedImage(target.width, target.height, BufferedImage.TYPE_INT_RGB);
            this.diff = new BufferedImage(target.width, target.height, BufferedImage.TYPE_INT_RGB);
            ...
            width = EXTRA_WIDTH + 2 * target.width;
            height = EXTRA_HEIGHT + 2 * target.height;
    ...
    double runTest() throws Exception {
        ...
        Image collage = new Image(tc.target.height, tc.target.width);
        for (int i=0; i < tc.target.height; i++) {
        ...
        for (int i = 0; i < tc.SOURCE_IMAGE_COUNT; i++) {
            ...
            if (fromR < 0 || fromR >= tc.target.height) {
                System.err.println("ERROR: top row for " + i + "-th source image (0-based) must be from 0" +
                        " to " + (tc.target.height-1) + ", inclusive. In your return value it is equal to " + fromR + ".");
                return -1;
            }

            if (toR < 0 || toR >= tc.target.height) {
                System.err.println("ERROR: bottom row for " + i + "-th source image (0-based) must be from 0" +
                        " to " + (tc.target.height-1) + ", inclusive. In your return value it is equal to " + toR + ".");
                return -1;
            }

            if (fromC < 0 || fromC >= tc.target.width) {
                System.err.println("ERROR: left column for " + i + "-th source image (0-based) must be from 0" +
                        " to " + (tc.target.width-1) + ", inclusive. In your return value it is equal to " + fromC + ".");
                return -1;
            }

            if (toC < 0 || toC >= tc.target.width) {
                System.err.println("ERROR: right column for " + i + "-th source image (0-based) must be from 0" +
                        " to " + (tc.target.width-1) + ", inclusive. In your return value it is equal to " + toC + ".");
                return -1;
            }
        ...
        return Math.sqrt(score / (tc.target.height * tc.target.width));
    }
